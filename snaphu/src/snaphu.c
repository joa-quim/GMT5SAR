...
void UnwrapTile(infileT *infiles, outfileT *outfiles, paramT *params, 
	...
	ReadInputFile(infiles,&mag,&wrappedphase,&flows,linelen,nlines, params,tileparams);
	...
	if(strlen(infiles->estfile)){
		ReadUnwrappedEstimateFile(&unwrappedest,infiles,linelen,nlines, params,tileparams);
	...
	if(!params->unwrapped){
		...
		if(params->initmethod==MSTINIT){
			...
			MSTInitFlows(wrappedphase,&flows,mstcosts,nrow,ncol, &nodes,ground,params->initmaxflow);
		
		}else if(params->initmethod==MCFINIT){

			/* use minimum cost flow (MCF) algorithm */
			MCFInitFlows(wrappedphase,&flows,mstcosts,nrow,ncol, params->cs2scalefactor);
	...
	while(TRUE){ 
		...
		SetupIncrFlowCosts(costs,incrcosts,flows,nflow,nrow,narcrow,narcsperrow, params); 
		...
		source=SelectSource(nodes,ground,nflow,flows,ngroundarcs, nrow,ncol,params);
