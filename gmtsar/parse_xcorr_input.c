...
void handle_prm(char **argv, struct xcorr *xc, int nfiles) {
	...
	filename = calloc(nfiles, sizeof(char *));
	for (i=0; i<nfiles; i++) filename[i] = calloc(128, sizeof(char));

	r = calloc(nfiles, sizeof(struct PRM));

	for (i=0; i<nfiles; i++){
		strcpy(filename[i], argv[i + 1]);		/* A PRM file (ASCII) */
		...
		if (i == 0) {
			...
			if ((xc->data1 = fopen(xc->data1_name,"rb")) == NULL) die("Cannot open SLC_file",xc->data1_name);
		...
		if (i == 1) {
			...
			//xc->data2 = fopen(xc->data2_name,"r");	/* It was opened twice JL */
			if ((xc->data2 = fopen(xc->data2_name,"rb")) == NULL) die("Cannot open SLC_file",xc->data2_name);
