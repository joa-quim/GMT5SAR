...
void read_optional_args(void *API, int argc, char **argv, struct PRM *tp, int *topoflag, struct PRM *mp, int *modelflag) {
	...
	for (i=3; i<argc; i++) {
		if (strcmp(argv[i],"-topo") == 0) {
			...
			if ((T = GMT_Read_Data (API, GMT_IS_GRID, GMT_IS_FILE, GMT_IS_SURFACE, GMT_GRID_HEADER_ONLY, NULL, tp->input_file, NULL)) == NULL)
		...
		if (strcmp(argv[i],"-model") == 0) {
			...
			if ((M = GMT_Read_Data (API, GMT_IS_GRID, GMT_IS_FILE, GMT_IS_SURFACE, GMT_GRID_HEADER_ONLY, NULL, mp->input_file, NULL)) == NULL)
...
int main (int argc, char **argv) {
	...
	/* open SLC files */
	if ((SLCfile1 = fopen(p1.SLC_file,"rb")) == NULL) die("Can't open SLCfile",p1.SLC_file); 
	if ((SLCfile2 = fopen(p2.SLC_file,"rb")) == NULL) die("Can't open SLCfile",p2.SLC_file); 
