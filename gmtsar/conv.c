...
int determine_file_type(char *name, int *input_file_type) {
	...
	*input_file_type = 1;
	...
	if (verbose && *input_file_type == 1) fprintf(stderr," input: GMT binary\n");
...
int main(int argc, char **argv) {
	...
	/* size of output file */
	iout = jout = 0;
	for (ic = 0; ic < ydim; ic = ic+idec) iout++;
	for (jc = 0; jc < xdim; jc = jc+jdec) jout++;
	...
	if ((Out = GMT_Create_Data (API, GMT_IS_GRID, GMT_IS_SURFACE, GMT_GRID_ALL, NULL, wesn, inc,
	                            GMT_GRID_PIXEL_REG, 0, NULL)) == NULL)
	...
	norm = 0;
	if (fabs(rnormax) > 0.05*anormax) norm = 1;
	ic0 = 0;
	iend = ylen = ibuff;
	...
	for (ic = 0, row = 0; ic < ydim; ic = ic+idec) {
		...
		if (verbose && (ic/2000.0 == ic/1000)) fprintf(stderr," line %d\n",ic);
