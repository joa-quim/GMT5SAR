...
#ifndef __CYGWIN__
#ifdef _WIN32
static _inline unsigned short bswap_16(unsigned short x) {
	return (x >> 8) | (x << 8);
}

static _inline unsigned int bswap_32(unsigned int x) {
...
int main(int argc, char **argv){
	...
	xml_tree = (struct tree *)calloc(n*5,sizeof(struct tree));
	fclose(XML_FILE);
...
int write_slc(FILE *input, FILE *slc, int rows, int cols){
	...
	int i,j,tj,k,tk,x;
	short *buf = calloc(sizeof(unsigned short),(cols+2)*2);
