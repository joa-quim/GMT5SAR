...
struct 	PRM r;
	...
	if((indata = (short int *) calloc(ibufsize,sizeof(short int))) == NULL){
	...
	if((outdata = (short int *) calloc(obufsize,sizeof(short int))) == NULL){
		fprintf(stderr,"Sorry, couldn't allocate memory for output outdata.\n");
		exit(-1);
        }
	
	if((extr = (short int *) calloc(N , sizeof(short int))) == NULL){
	...
	if((cin = (fcomplex *) calloc(nffti,sizeof(fcomplex))) == NULL){
		fprintf(stderr,"Sorry, couldn't allocate memory for fbd \n");
		exit(-1);
	}
	if((cout = (fcomplex *) calloc(nffto,sizeof(fcomplex))) == NULL){
