...
/* allocate storage for an array of pointers  */
        orb_pos = calloc(4,sizeof(double *));
        
/* for each pointer, allocate storage for an array of floats  */
        for(j=0; j<4; j++) {
          orb_pos[j] = calloc((nrec+2*npad) , sizeof(double));
        }
...

{
	...
	double time;
        ...
        px = (double *) calloc(orb->nd,sizeof(double));
	py = (double *) calloc(orb->nd,sizeof(double));
	pz = (double *) calloc(orb->nd,sizeof(double));
        pvx = (double *) calloc(orb->nd,sizeof(double));
	pvy = (double *) calloc(orb->nd,sizeof(double));
	pvz = (double *) calloc(orb->nd,sizeof(double));
        pt = (double *) calloc(orb->nd,sizeof(double));
