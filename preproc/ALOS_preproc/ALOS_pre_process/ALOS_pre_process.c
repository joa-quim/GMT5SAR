...
int main (int argc, char **argv) {
	...
	if ((imagefile = fopen(argv[1], "rb")) == NULL) die ("couldn't open Level 1.0 IMG file \n",argv[1]);
	...
	while (byte_offset != 0) {
		...
		/* read_ALOS_data returns 0 if all data file is read; returns byte offset if the PRF changes  */
