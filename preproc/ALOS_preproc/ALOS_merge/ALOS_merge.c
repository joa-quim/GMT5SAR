...
struct PRM p3;			
	...
	/* open raw files */
	if ((RAWfile1 = fopen(p1.input_file,"rb")) == NULL) die("cannot open ",p1.input_file);
	if ((RAWfile2 = fopen(p2.input_file,"rb")) == NULL) die("cannot open ",p2.input_file);

	/* output */
	if ((RAWfile3 = fopen(RawOutName,"wb")) == NULL) die("cannot open output file ",RawOutName);
...
int64_t read_ALOS_raw_data (FILE *imagefile, FILE *outfile, struct PRM *prm, int *start_frame) {
	...
	data = calloc(line_data_size , sizeof(char));
