...
void read_ALOS_sarleader(FILE *ldrfile, struct PRM *prm, struct ALOS_ORB *orb) {
	...
	*/
	...
	sar.position_ALOS = (struct position_vector_ALOS *) calloc(num_orbit_points, sizeof(struct position_vector_ALOS));
	...
	if (verbose) fprintf(stderr,".... reading sarleader %d\n",num_att_points);
	sar.attitude_ALOS = (struct attitude_data_ALOS *) calloc(num_att_points , sizeof(struct attitude_data_ALOS));
...
void print_binary_position(struct sarleader_binary *sb, int nitems, FILE *ldrfile, FILE *logfile) {
	...
	fprintf(logfile," read %d items (%zd bytes) at position %ld\n", nitems, sizeof(struct sarleader_binary), ftell(ldrfile));
}
/*---------------------------------------------------------------*/
/* write a PRM file */
...
char	tmp[256];
	...
	orb->points = (struct ORB_XYZ *) calloc(orb->nd,sizeof(struct ORB_XYZ));
