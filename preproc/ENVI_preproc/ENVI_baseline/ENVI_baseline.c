...
/*---------------------------------------------------------------------------*/
static void parse_command_line_local(char **argv, int *nfiles, int *input_flag) {
	char  tmp[132];
	FILE *inputfile;
...
/*---------------------------------------------------------------------------*/
static void write_bperp(struct PRM r1, char *string) {
	FILE	*GMTscript;
	float	year, dyear;
...
/*---------------------------------------------------------------------------*/
static void get_sign(struct PRM r, double x11, double y11, double x21, double y21, int *sign) {
...
/*---------------------------------------------------------------------------*/
static void write_prm_baseline(struct PRM rep) {
	printf("SC_vel			= %f \n",rep.vel);
	printf("SC_height		= %f \n",rep.ht);
...
/*---------------------------------------------------------------------------*/
static void find_parallel_perp_baseline(struct PRM ref, struct PRM rep, double dr, double *bpara, double *bperp) {
...
/*---------------------------------------------------------------------------*/
static double find_alpha_degrees(double bv, double bh) {
	double a, rad;
...
/*---------------------------------------------------------------------------*/
static void endpoint_distance(int k, double ds, double xs, double ys, double zs, double *b, double *x, double *y, double *z, int *m) {
	*b = ds;
	*x = xs;
	*y = ys;
	*z = zs;
...
/*---------------------------------------------------------------------------*/
static void find_unit_vectors(double x, double y, double z, double *ru, double *xu, double *yu, double *zu) {
	*ru = sqrt(x*x + y*y + z*z);
	*xu = x/(*ru);
	*yu = y/(*ru);
	*zu = z/(*ru);
...
void read_all_ENVI_ldr(struct PRM *r, struct ALOS_ORB *orb, int nfiles) {
        ...
        for (i=0; i<nfiles; i++){
                ...
                if ((ldrfile = fopen(r[i].led_file,"rb")) == NULL) die("Can't open ldrfile %s",r[i].led_file);
...
void ENVI_baseline(struct PRM *r, struct ALOS_ORB *orb, int nfiles, int input_flag, char **filename,double fs0) {
	...
	nd = orb[0].nd;
	pt = calloc(nd,sizeof(double));
	p = calloc(nd,sizeof(double));
	pv = calloc(nd,sizeof(double));
